...
<head>
    ...
    <link rel="stylesheet" href="/css/site/app.css?ver={{filemtime('css/site/app.css')}} crossorigin="anonymous" async>
    @meta_tags
...
<div id="app">
  <industries-layout
    @isset($portfolio) :portfolio="{{ json_encode($portfolio) }}" @endisset
    @isset($learn) :learn="{{ json_encode($learn) }}" @endisset
    @isset($connect) :connect="{{ json_encode($connect) }}" @endisset
  >
    @yield('content')
  </industries-layout>
</div>
